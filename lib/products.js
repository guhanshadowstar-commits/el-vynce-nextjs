/* EL VYNCE — product catalogue with real size charts.
   Size charts sourced from Print On Demand India (Qikinki platform). */

export const EL_VYNCE_PRODUCTS = [
  {
    id: "ev-002",
    name: "Rebel Soul",
    drop: "Emotional Drops",
    price: 899,
    currency: "INR",
    colors: ["#b3001b"],
    sizes: ["XS", "S", "M", "L", "XL", "2XL"],
    sizeChart: {
      XS: { chest: "41\"", length: "27\"", shoulder: "20\"", sleeve: "7.75\"" },
      S: { chest: "43\"", length: "27.5\"", shoulder: "21\"", sleeve: "8\"" },
      M: { chest: "45\"", length: "28\"", shoulder: "22\"", sleeve: "8.5\"" },
      L: { chest: "47\"", length: "28.5\"", shoulder: "22.5\"", sleeve: "9\"" },
      XL: { chest: "49\"", length: "29\"", shoulder: "23\"", sleeve: "9.5\"" },
      "2XL": { chest: "51\"", length: "30\"", shoulder: "24\"", sleeve: "9.5\"" },
    },
    images: ["/images/products/rebel-soul-front.jpg", "/images/products/rebel-soul-back.jpg"],
    placeholderSwatch: {
      bg: "#b3001b",
      front: { text: "REBEL SOUL", style: "serif", color: "#ffffff" },
      back: { text: "I stayed quiet, that was my loudest rebellion", style: "line-art", color: "#ffffff" },
    },
    description:
      'Red tee — front print "REBEL SOUL" in white serif text. Back print: abstract line-art of overlapping face/profile silhouettes with the line "I stayed quiet, that was my loudest rebellion." Made to order in our atelier.',
    inStock: true,
    isNew: true,
  },
  {
    id: "ev-003",
    name: "Frequency",
    drop: "Emotional Drops",
    price: 899,
    currency: "INR",
    colors: ["#d8c3a5"],
    sizes: ["XS", "S", "M", "L", "XL", "2XL"],
    sizeChart: {
      XS: { chest: "41\"", length: "27\"", shoulder: "20\"", sleeve: "7.75\"" },
      S: { chest: "43\"", length: "27.5\"", shoulder: "21\"", sleeve: "8\"" },
      M: { chest: "45\"", length: "28\"", shoulder: "22\"", sleeve: "8.5\"" },
      L: { chest: "47\"", length: "28.5\"", shoulder: "22.5\"", sleeve: "9\"" },
      XL: { chest: "49\"", length: "29\"", shoulder: "23\"", sleeve: "9.5\"" },
      "2XL": { chest: "51\"", length: "30\"", shoulder: "24\"", sleeve: "9.5\"" },
    },
    images: ["/images/products/frequency-front.jpg", "/images/products/frequency-back.jpg"],
    placeholderSwatch: {
      bg: "#d8c3a5",
      front: { text: "FREQUENCY", style: "thin-sans", color: "#000000" },
      back: {
        text: "Everything is energy and a vibrational frequency that includes you",
        style: "sacred-geometry",
        color: "#000000",
      },
    },
    description:
      'Tan/beige tee — front print "FREQUENCY" in a thin elegant sans-serif wordmark. Back print: a vertical column of geometric/sacred-geometry symbols (triangle, bowtie, cone, sphere, rings, cube) with the line "Everything is energy and a vibrational frequency that includes you."',
    inStock: true,
    isNew: true,
  },
  {
    id: "ev-003b",
    name: "Inner Noise",
    drop: "Emotional Drops",
    price: 899,
    currency: "INR",
    colors: ["#0e6b73"],
    sizes: ["XS", "S", "M", "L", "XL", "2XL"],
    sizeChart: {
      XS: { chest: "41\"", length: "27\"", shoulder: "20\"", sleeve: "7.75\"" },
      S: { chest: "43\"", length: "27.5\"", shoulder: "21\"", sleeve: "8\"" },
      M: { chest: "45\"", length: "28\"", shoulder: "22\"", sleeve: "8.5\"" },
      L: { chest: "47\"", length: "28.5\"", shoulder: "22.5\"", sleeve: "9\"" },
      XL: { chest: "49\"", length: "29\"", shoulder: "23\"", sleeve: "9.5\"" },
      "2XL": { chest: "51\"", length: "30\"", shoulder: "24\"", sleeve: "9.5\"" },
    },
    images: ["/images/products/inner-noise-front.jpg", "/images/products/inner-noise-back.jpg"],
    description:
      'Teal tee — front print "INNER NOISE" in a tall serif wordmark. Back print: an illustrated profile portrait wrapped in looping arrow-paths with small surrounding captions, with "INNER NOISE" set across the chest.',
    inStock: true,
    isNew: true,
  },
  {
    id: "ev-004",
    name: "Style Pays Off",
    drop: "Warrior Drop",
    price: 599,
    currency: "INR",
    colors: ["#f1ead9"],
    sizes: ["S", "M", "L", "XL", "2XL"],
    sizeChart: {
      S: { chest: "42\"", length: "27.5\"", shoulder: "20\"", sleeve: "8.5\"" },
      M: { chest: "44\"", length: "28\"", shoulder: "21\"", sleeve: "9\"" },
      L: { chest: "46\"", length: "28.5\"", shoulder: "22\"", sleeve: "9.5\"" },
      XL: { chest: "48\"", length: "29\"", shoulder: "23\"", sleeve: "10\"" },
      "2XL": { chest: "50\"", length: "29.5\"", shoulder: "24\"", sleeve: "10.5\"" },
    },
    images: ["/images/products/style-pays-off-front.jpg"],
    placeholderSwatch: {
      bg: "#f1ead9",
      front: { text: "STYLE PAYS OFF", style: "barcode", color: "#000000" },
    },
    description:
      'Cream/off-white tee — front print: a barcode graphic with "STYLE PAYS OFF" set beneath it. Part of the Warrior Drop.',
    inStock: true,
    isNew: true,
  },
  {
    id: "ev-005",
    name: "Just Be Resilient",
    drop: "Warrior Drop",
    price: 599,
    currency: "INR",
    colors: ["#1b2a4a"],
    sizes: ["S", "M", "L", "XL", "2XL"],
    sizeChart: {
      S: { chest: "42\"", length: "27.5\"", shoulder: "20\"", sleeve: "8.5\"" },
      M: { chest: "44\"", length: "28\"", shoulder: "21\"", sleeve: "9\"" },
      L: { chest: "46\"", length: "28.5\"", shoulder: "22\"", sleeve: "9.5\"" },
      XL: { chest: "48\"", length: "29\"", shoulder: "23\"", sleeve: "10\"" },
      "2XL": { chest: "50\"", length: "29.5\"", shoulder: "24\"", sleeve: "10.5\"" },
    },
    images: ["/images/products/just-be-resilient-front.jpg", "/images/products/just-be-resilient-back.jpg"],
    placeholderSwatch: {
      bg: "#1b2a4a",
      front: { text: "JUST BE RESILIENT", style: "elegant-serif", color: "#ffffff" },
      back: {
        text: "RESILIENT / facing difficulties head-on / TAKE CONTROL, LIVE YOUR DREAMS / El Vynce",
        style: "bold-sans-plus-script",
        color: "#ffffff",
      },
    },
    description:
      'Navy tee — front print "JUST BE RESILIENT" in elegant serif. Back print: "RESILIENT" in bold sans-serif, with sub-text "facing difficulties head-on" and "TAKE CONTROL, LIVE YOUR DREAMS," plus the El Vynce signature-script logo.',
    inStock: true,
    isNew: true,
  },
  {
    id: "ev-006",
    name: "Dare to Be Different",
    drop: "Warrior Drop",
    price: 599,
    currency: "INR",
    colors: ["#000000"],
    sizes: ["S", "M", "L", "XL", "2XL"],
    sizeChart: {
      S: { chest: "42\"", length: "27.5\"", shoulder: "20\"", sleeve: "8.5\"" },
      M: { chest: "44\"", length: "28\"", shoulder: "21\"", sleeve: "9\"" },
      L: { chest: "46\"", length: "28.5\"", shoulder: "22\"", sleeve: "9.5\"" },
      XL: { chest: "48\"", length: "29\"", shoulder: "23\"", sleeve: "10\"" },
      "2XL": { chest: "50\"", length: "29.5\"", shoulder: "24\"", sleeve: "10.5\"" },
    },
    images: ["/images/products/dare-to-be-different-front.jpg", "/images/products/dare-to-be-different-back.jpg"],
    placeholderSwatch: {
      bg: "#000000",
      front: {
        text: "Dare to be different. Anything is possible if you believe. The future is ours",
        style: "hand-drawn-scribble",
        color: "#ffffff",
      },
      back: {
        text: "UNREAL / Exclusive Members Only (DESIGNED AND DEVELOPED) / El Vynce",
        style: "collegiate-bold-plus-script",
        color: "#ffffff",
      },
    },
    description:
      'Black tee — front print: a hand-drawn-style squiggle/scribble wordmark with small text along it: "Dare to be different. Anything is possible if you believe. The future is ours." Back print: "UNREAL" in bold collegiate lettering with "Exclusive Members Only (DESIGNED AND DEVELOPED)" and the El Vynce signature-script logo.',
    inStock: true,
    isNew: false,
  },
  {
    id: "ev-006b",
    name: "Built Different",
    drop: "Warrior Drop",
    price: 599,
    currency: "INR",
    colors: ["#8a9a7b"],
    sizes: ["S", "M", "L", "XL", "2XL"],
    sizeChart: {
      S: { chest: "42\"", length: "27.5\"", shoulder: "20\"", sleeve: "8.5\"" },
      M: { chest: "44\"", length: "28\"", shoulder: "21\"", sleeve: "9\"" },
      L: { chest: "46\"", length: "28.5\"", shoulder: "22\"", sleeve: "9.5\"" },
      XL: { chest: "48\"", length: "29\"", shoulder: "23\"", sleeve: "10\"" },
      "2XL": { chest: "50\"", length: "29.5\"", shoulder: "24\"", sleeve: "10.5\"" },
    },
    images: ["/images/products/built-different-front.jpg", "/images/products/built-different-back.jpg"],
    placeholderSwatch: {
      bg: "#8a9a7b",
      front: { text: "Built Different", style: "gothic-blackletter", color: "#000000" },
      back: {
        text: "COMFORT ZONES ARE OVERRATED / Built Different / ESTD.2025",
        style: "tall-serif-caps-plus-script",
        color: "#000000",
      },
    },
    description:
      'Sage green tee — front print "Built Different" in gothic/blackletter font. Back print: "COMFORT ZONES ARE OVERRATED" in tall serif caps, overlaid with the script "Built Different" and "ESTD.2025."',
    inStock: true,
    isNew: false,
  },
  {
    id: "ev-006c",
    name: "I'm Just a Girl",
    drop: "Crop Tops",
    price: 399,
    currency: "INR",
    colors: ["#c9a8c4"],
    sizes: ["XS", "S", "M", "L", "XL", "2XL"],
    sizeChart: {
      XS: { chest: "32\"", length: "15.5\"" },
      S: { chest: "34\"", length: "16.5\"" },
      M: { chest: "36\"", length: "17.5\"" },
      L: { chest: "38\"", length: "18.5\"" },
      XL: { chest: "40\"", length: "19.5\"" },
      "2XL": { chest: "42\"", length: "20.5\"" },
    },
    images: ["/images/products/im-just-a-girl-front.jpg"],
    femaleOnly: true,
    description:
      'Lavender crop top — front print: a pink bow graphic with the line "I\'m just a girl."',
    inStock: true,
    isNew: true,
  },
  {
    id: "ev-009",
    name: "She.",
    drop: "Crop Tops",
    price: 399,
    currency: "INR",
    colors: ["#ece1c6"],
    sizes: ["XS", "S", "M", "L", "XL", "2XL"],
    sizeChart: {
      XS: { chest: "32\"", length: "15.5\"" },
      S: { chest: "34\"", length: "16.5\"" },
      M: { chest: "36\"", length: "17.5\"" },
      L: { chest: "38\"", length: "18.5\"" },
      XL: { chest: "40\"", length: "19.5\"" },
      "2XL": { chest: "42\"", length: "20.5\"" },
    },
    images: ["/images/products/she-front.jpg"],
    femaleOnly: true,
    description: 'Khaki crop top — front print "She." in a bold red script serif.',
    inStock: true,
    isNew: true,
  },
  {
    id: "ev-010",
    name: "Pretty Girls Don't Do Drama",
    drop: "Crop Tops",
    price: 399,
    currency: "INR",
    colors: ["#ecd8dc"],
    sizes: ["XS", "S", "M", "L", "XL", "2XL"],
    sizeChart: {
      XS: { chest: "32\"", length: "15.5\"" },
      S: { chest: "34\"", length: "16.5\"" },
      M: { chest: "36\"", length: "17.5\"" },
      L: { chest: "38\"", length: "18.5\"" },
      XL: { chest: "40\"", length: "19.5\"" },
      "2XL": { chest: "42\"", length: "20.5\"" },
    },
    images: ["/images/products/pretty-girls-dont-do-drama-front.jpg"],
    femaleOnly: true,
    description: 'Soft pink crop top — front print "Pretty Girls Don\'t Do Drama" in a playful hand-drawn caps font.',
    inStock: true,
    isNew: true,
  },
  {
    id: "ev-011",
    name: "Spicy",
    drop: "Crop Tops",
    price: 399,
    currency: "INR",
    colors: ["#ffffff"],
    sizes: ["XS", "S", "M", "L", "XL", "2XL"],
    sizeChart: {
      XS: { chest: "32\"", length: "15.5\"" },
      S: { chest: "34\"", length: "16.5\"" },
      M: { chest: "36\"", length: "17.5\"" },
      L: { chest: "38\"", length: "18.5\"" },
      XL: { chest: "40\"", length: "19.5\"" },
      "2XL": { chest: "42\"", length: "20.5\"" },
    },
    images: ["/images/products/spicy-front.jpg"],
    femaleOnly: true,
    description: 'White crop top — front print "spicy" in bold red lowercase lettering, the dot on the "i" styled as a chili pepper.',
    inStock: true,
    isNew: true,
  },
  {
    id: "ev-012",
    name: "Cream Waffle Knit",
    drop: "Knit-wear Drops",
    price: 799,
    currency: "INR",
    colors: ["#f2ecd8"],
    sizes: ["S", "M", "L", "XL", "2XL"],
    sizeChart: {
      S: { length: "24cm", chest: "45cm", shoulder: "23cm", sleeve: "20.5cm" },
      M: { length: "25cm", chest: "47cm", shoulder: "24cm", sleeve: "21.5cm" },
      L: { length: "26cm", chest: "49cm", shoulder: "25cm", sleeve: "22.5cm" },
      XL: { length: "27cm", chest: "51cm", shoulder: "26cm", sleeve: "23.5cm" },
      "2XL": { length: "28cm", chest: "53cm", shoulder: "27cm", sleeve: "24.5cm" },
    },
    images: ["/images/products/knit-cream-front.jpg"],
    description:
      "Cream waffle-knit long sleeve with the EL VYNCE script logo at the chest. Oversized fit, heavyweight textured cotton.",
    inStock: true,
    isNew: true,
  },
  {
    id: "ev-013",
    name: "Black Waffle Knit",
    drop: "Knit-wear Drops",
    price: 799,
    currency: "INR",
    colors: ["#1a1a1a"],
    sizes: ["S", "M", "L", "XL", "2XL"],
    sizeChart: {
      S: { length: "24cm", chest: "45cm", shoulder: "23cm", sleeve: "20.5cm" },
      M: { length: "25cm", chest: "47cm", shoulder: "24cm", sleeve: "21.5cm" },
      L: { length: "26cm", chest: "49cm", shoulder: "25cm", sleeve: "22.5cm" },
      XL: { length: "27cm", chest: "51cm", shoulder: "26cm", sleeve: "23.5cm" },
      "2XL": { length: "28cm", chest: "53cm", shoulder: "27cm", sleeve: "24.5cm" },
    },
    images: ["/images/products/knit-black-front.jpg"],
    description:
      "Black waffle-knit long sleeve with the EL VYNCE script logo at the chest. Oversized fit, heavyweight textured cotton.",
    inStock: true,
    isNew: true,
  },
  {
    id: "ev-014",
    name: "Brown Waffle Knit",
    drop: "Knit-wear Drops",
    price: 799,
    currency: "INR",
    colors: ["#6b4536"],
    sizes: ["S", "M", "L", "XL", "2XL"],
    sizeChart: {
      S: { length: "24cm", chest: "45cm", shoulder: "23cm", sleeve: "20.5cm" },
      M: { length: "25cm", chest: "47cm", shoulder: "24cm", sleeve: "21.5cm" },
      L: { length: "26cm", chest: "49cm", shoulder: "25cm", sleeve: "22.5cm" },
      XL: { length: "27cm", chest: "51cm", shoulder: "26cm", sleeve: "23.5cm" },
      "2XL": { length: "28cm", chest: "53cm", shoulder: "27cm", sleeve: "24.5cm" },
    },
    images: ["/images/products/knit-brown-front.jpg"],
    description:
      "Brown waffle-knit long sleeve with the EL VYNCE script logo at the chest. Oversized fit, heavyweight textured cotton.",
    inStock: true,
    isNew: true,
  },
  {
    id: "ev-015",
    name: "Grey Waffle Knit",
    drop: "Knit-wear Drops",
    price: 799,
    currency: "INR",
    colors: ["#8a8a8a"],
    sizes: ["S", "M", "L", "XL", "2XL"],
    sizeChart: {
      S: { length: "24cm", chest: "45cm", shoulder: "23cm", sleeve: "20.5cm" },
      M: { length: "25cm", chest: "47cm", shoulder: "24cm", sleeve: "21.5cm" },
      L: { length: "26cm", chest: "49cm", shoulder: "25cm", sleeve: "22.5cm" },
      XL: { length: "27cm", chest: "51cm", shoulder: "26cm", sleeve: "23.5cm" },
      "2XL": { length: "28cm", chest: "53cm", shoulder: "27cm", sleeve: "24.5cm" },
    },
    images: ["/images/products/knit-grey-front.jpg"],
    description:
      "Grey waffle-knit long sleeve with the EL VYNCE script logo at the chest. Oversized fit, heavyweight textured cotton.",
    inStock: true,
    isNew: true,
  },
  {
    id: "test-payment-1rs",
    name: "Payment Test — ₹1",
    drop: "Internal Test",
    price: 1,
    currency: "INR",
    colors: ["#666666"],
    sizes: ["M"],
    images: [],
    placeholderSwatch: {
      bg: "#666666",
      front: { text: "PAYMENT TEST", style: "thin-sans", color: "#ffffff" },
    },
    description: "Internal ₹1 order used to verify the live Razorpay + CRM pipeline. Not a real product — delete after testing.",
    inStock: true,
    isNew: false,
    hidden: true,
  },
  {
    id: "test-payment-1rs-2",
    name: "Payment Test 2 — ₹1",
    drop: "Internal Test",
    price: 1,
    currency: "INR",
    colors: ["#8a5a2b"],
    sizes: ["M"],
    images: [],
    placeholderSwatch: {
      bg: "#8a5a2b",
      front: { text: "CRM RETEST", style: "thin-sans", color: "#ffffff" },
    },
    description: "Second internal ₹1 order used to re-verify the Notion CRM write after fixing the corrupted NOTION_TOKEN. Not a real product — delete after testing.",
    inStock: true,
    isNew: false,
    hidden: true,
  },
];

export function getProductById(id) {
  return EL_VYNCE_PRODUCTS.find((p) => p.id === id) || null;
}
