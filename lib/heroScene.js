...
export function initHeroScene(mount) {
  ...
  function createCar(bodyColor) {
    ...
    return { group, wheels, hlMat, tlMat };
  }
  ...
  CAR_LANES.forEach(({ x, dir }) => {
    ...
    for (let i = 0; i < CAR_PER_LANE; i++) {
      ...
      car._goAt = null;
      car.state = "normal"; // normal | tapStop | distracted | crashed
      car.honkUntil = 0;
      car.hazardUntil = 0;
    ...
    cars.forEach((car) => {
      ...
      if (car.state === "distracted") {
        ...
        if (!lead || lead.state === "normal") {
          ...
          const contact = CAR_HALF + CAR_HALF + 0.02;
          if (gap <= contact) {
      ...
      if (car.state !== "distracted") {
        ...
        const stopGap = CAR_HALF + CAR_HALF + 0.3;
