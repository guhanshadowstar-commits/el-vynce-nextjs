/* EL VYNCE — "Rush Hour, 8:59 AM" cinematic city hero.
   ...
   sitter. Time of day AND crowd density are driven by the visitor's real
...
export function initHeroScene(mount) {
  ...
  // Commuter walkers (the rush-hour stream). Dancer and bench sitter are added on top of these.
  ...
  function createCar(bodyColor) {
    ...
    const wheels = [];
    // Wheels don't cast their own shadow — small, low, already inside the
    // body's shadow footprint, so the extra casters buy no visible
    // difference for real shadow-pass cost.
  ...
  function stepCars(t, dt, sig) {
    ...
    cars.forEach((car) => {
      ...
      car._wasGreen = green;
      const effectiveStop = green || (car._goAt != null && t < car._goAt);

      let target = car.cruise;

      // Yield to the crosswalk while pedestrians cross (or stagger delay active).
      if (effectiveStop) {
  ...
  Promise.all([
    ...
    // Each walker's productId is set from the outfit actually worn, so
    // clicking a figure opens the exact product on their chest.
    ...
    const sitterOutfit = outfits[7]; // Style Pays Off — unisex, and matches her click-through below
    ...
    sitter.group.rotation.y = Math.PI * 0.55;
    ...
    scene.add(bench);
    if (__debug) __debug.sitter = sitter;
  }).catch((err) => {
    if (disposed) return;
  ...
  const onClick = (e) => {
    ...
    if (hit) window.location.href = `/product/${hit.productId}`;
  };
